...
class AdminRepository
{
    ...
    {
        $sql = "SELECT count(*) as num FROM users WHERE is_active=1";
        return $this->db->query($sql)->row['num']??0;
    }

    public function getTableSize($dbname)
    {
        $sql = "SELECT COUNT(*) as table_count FROM information_schema.tables WHERE table_schema = :dbname";
        $this->db->setData([':dbname'=>$dbname]);
        return $this->db->query($sql)->row['table_count']??0;
    }
}
